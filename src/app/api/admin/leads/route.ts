...
import { getLeads, getDemoBookings, getStudents, getCounsellingRecords, getNotificationLogs, getTimeSlots, updateLeadStatus as updateProductionLeadStatus } from '@/lib/productionDb';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (process.env.DATABASE_URL) {
      const [leads, demoBookings, students, counselling, notifications, timeSlots] = await Promise.all([
        getLeads(), getDemoBookings(), getStudents(), getCounsellingRecords(), getNotificationLogs(), getTimeSlots(),
      ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'update_status') {
      ...
      if (process.env.DATABASE_URL) {
        const updated = await updateProductionLeadStatus(leadId, status as never, body?.counsellor, body?.notes);
