...
import { getStudentByEmail, updateStudentPassword } from '@/lib/productionDb';
...
export async function POST(req: Request) {
  try {
    ...
    if (process.env.DATABASE_URL) {
      ...
      if (action === 'register') {
        if (!student) {
          return NextResponse.json({ success: false, message: 'Complete student registration before creating login credentials.' }, { status: 404 });
        }
        if (password.length < 6) {
          return NextResponse.json({ success: false, message: 'Password must contain at least 6 characters.' }, { status: 400 });
        }

        const updated = await updateStudentPassword(student.studentId, bcrypt.hashSync(password, 12));
        if (!updated) {
          return NextResponse.json({ success: false, message: 'Unable to create student credentials.' }, { status: 500 });
        }

        const result = await authManager.loginStudentAsync(email, password, deviceInfo);
    ...
    // Local fallback: Excel remains available for development only.
    ...
    return NextResponse.json({ success: false, message: 'Student authentication failed. Please try again.' }, { status: 500 });
  }
}
